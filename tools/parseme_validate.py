...
#
# parseme_validate.py is part of parseme-utilities
#
...
UD_VALIDATE = f"{THISDIR}/valid_ud/validate.py"
...
def load_languages_set(filename):
    ...
    """
    res = load_file(os.path.join(THISDIR, 'valid_parseme', filename))
    return res
...
def load_mwe_set(filename, lcode):
    ...
    """
    res = load_file(os.path.join(THISDIR, 'valid_parseme', filename))
    with open(os.path.join(THISDIR, 'valid_parseme', filename), 'r', encoding='utf-8') as f:
...
sentid_re=re.compile(r'^# source_sent_id\s*=\s*(\S+)\s+(\S+)\s+(\S+)$')
...
metadata_re = re.compile(r'^#\s*metadata\s*=\s*')
...
def main():
    ...
    list_group.add_argument("--lang", action="store", required=True, default=None, help="Which langauge are we checking? If you specify this (as a two-letter code), the tags will be checked using the language-specific files in the valid_parseme/ directory of the validator.")
